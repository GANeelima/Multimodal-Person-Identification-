...
def get_dataset_iemocap(data_folder: str, phase: str, img_interval: int, hand_crafted_features: Optional[bool] = False):
    ...
    if hand_crafted_features:
        ...
        this_dataset = IEMOCAP_baseline(
            ...
            labels=labels,
            label_annotations=list(speakerID.keys()),
...
def collate_fn(batch):
    ...
    for dp in batch:
        utteranceId, sampledImgs, specgram, text, speaker_id = dp
...
class IEMOCAP_baseline(Dataset):
    ...
    def __getitem__(self, ind: int):
        ...
        speaker_id = self.labels[ind]

        return uttrId, video_feature, audio_feature, text, speaker_id
