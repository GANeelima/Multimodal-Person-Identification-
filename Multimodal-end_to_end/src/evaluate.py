...
def eval_iemocap_person(preds, truths, best_thresholds=None):
    # person_labels = ['Ses01F', 'Ses02F', ...]
    '''
    preds: (bs, num_persons)
    truths: (bs, num_persons)
    '''

    num_persons = preds.size(1)
    ...
    aucs = roc_auc_score(truths, preds, labels=list(range(num_persons)), average=None).tolist()
    aucs.append(np.average(aucs))

    if best_thresholds is None:
        # select the best threshold for each person category,based on F1 score
...
def eval_iemocap_ce_person(preds, truths):
    # person_labels = ['Ses01F', 'Ses02F', ...]
    '''
    preds: (num_of_data, num_persons)
    truths: (num_of_data,)
    '''
